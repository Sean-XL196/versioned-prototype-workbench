...
4. 在“开发规格”中切换本页已发布版本，确认预览标题、正文、下载文件名和文件内容同步更新，且默认选中当前版本。
5. 确认所选版本显示自身的基线提交、实现提交和 MR/PR；信息不完整时明确显示未关联，不生成虚假链接。
6. 当基线与实现提交均存在时，确认“查看本版 Diff”指向完整 SHA 组成的固定比较范围。
7. 在追溯矩阵中检查 `REQ-*`、`CHG-*`、`INT-*`、`RULE-*`、`TST-*` 等稳定 ID 及关联关系。
